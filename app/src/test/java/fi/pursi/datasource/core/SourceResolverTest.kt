...
private class TestWeatherProvider(
    ...
    ): List<fi.pursi.weather.StationWeatherData> = emptyList()
    override suspend fun getWaveObservations(
        latitude: Double, longitude: Double
    ): List<fi.pursi.weather.WaveStation> = emptyList()
    override suspend fun getForecast(
        latitude: Double, longitude: Double
    ): List<fi.pursi.weather.ForecastPoint> = emptyList()
    override suspend fun getLightningData(
        minLat: Double, minLng: Double, maxLat: Double, maxLng: Double
    ): List<fi.pursi.weather.LightningStrike> = emptyList()
}
...
private class TestWarningProvider(
    ...
    ): List<fi.pursi.weather.MarineWarning> = emptyList()
}
