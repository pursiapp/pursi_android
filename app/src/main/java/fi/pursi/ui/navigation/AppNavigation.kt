package fi.pursi.ui.navigation
...
import fi.pursi.R
import fi.pursi.location.LocationService
import fi.pursi.analytics.AnalyticsManager
import fi.pursi.ui.theme.PursiTheme
import fi.pursi.ui.theme.Dimens
import fi.pursi.ui.screens.MapScreen
import fi.pursi.ui.screens.RoutesScreen
import fi.pursi.ui.screens.RoutesContent
import fi.pursi.ui.screens.SettingsScreen
import fi.pursi.ui.screens.WeatherScreen
import fi.pursi.ui.screens.WeatherContent
import fi.pursi.ui.screens.AreaSelectionScreen
...
@Composable
fun AppNavigation(
    tileStorage: fi.pursi.map.TileStorage,
    downloadManager: fi.pursi.map.DownloadManager,
    networkMonitor: fi.pursi.map.NetworkMonitor,
    ...
    val locationPermissionLauncher = rememberLauncherForActivityResult(
        ...
        if (permissions.values.all { it }) {
            try {
                val intent = Intent(context, fi.pursi.location.LocationService::class.java)
        ...
        Surface(
            ...
            BoxWithConstraints(Modifier.fillMaxSize()) {
                ...
                val pmtilesDownloader = remember {
                    fi.pursi.map.PmtilesDownloader(context, okHttp)
                }
                val vvDataDownloader = remember {
                    fi.pursi.map.VvDataDownloader(context, okHttp)
                }
                ...
                val shared = SharedState(
                    ...
                    onClearWfsCache = { composeScope.launch(Dispatchers.IO) { fi.pursi.data.AppDatabase.getInstance(context).wfsFeatureDao().clearAll() } },
...
private data class SharedState(
    ...
    val tileStorage: fi.pursi.map.TileStorage,
    val downloadManager: fi.pursi.map.DownloadManager,
    val pmtilesDownloader: fi.pursi.map.PmtilesDownloader,
    val vvDataDownloader: fi.pursi.map.VvDataDownloader,
