...
import fi.pursi.map.GeocodingClient
import fi.pursi.map.OverpassClient
import fi.pursi.map.PoiCategory
import fi.pursi.map.PoiResult
import fi.pursi.map.SearchResult
import fi.pursi.navigation.RouteSimplifier
...
class RoutesViewModel @Inject constructor(
    ...
    fun getWaypointsForRoute(routeId: String, callback: (List<fi.pursi.data.model.RouteWaypoint>) -> Unit) {
        viewModelScope.launch {
    ...
    fun deleteRecording(recording: fi.pursi.data.model.TrackSummary) {
        viewModelScope.launch {
    ...
    fun exportGpx(route: SavedRoute) {
        viewModelScope.launch {
            try {
                ...
                context.startActivity(android.content.Intent.createChooser(shareIntent, context.getString(fi.pursi.R.string.export_gpx_title)))
            } catch (_: Exception) {
                _errorMessage.tryEmit(context.getString(fi.pursi.R.string.gpx_export_error))
            }
        }
    }
