...
import app.pursi.ui.viewmodel.SectorMode
...
import app.pursi.map.TileSourceBuilder
...
import app.pursi.map.parseRectanglesJson
...
fun SettingsScreen(
    ...
    navmarkSize: app.pursi.ui.viewmodel.NavmarkSize = app.pursi.ui.viewmodel.NavmarkSize.MEDIUM,
    onNavmarkSizeChange: (app.pursi.ui.viewmodel.NavmarkSize) -> Unit = {},
        ...
        item {
            SettingsCard {
                Row(
                    ...
                    val options = listOf(app.pursi.ui.viewmodel.NavmarkSize.SMALL, app.pursi.ui.viewmodel.NavmarkSize.MEDIUM, app.pursi.ui.viewmodel.NavmarkSize.LARGE)
                    options.forEach { opt ->
                        val selected = navmarkSize == opt
                        val label = when (opt) {
                            app.pursi.ui.viewmodel.NavmarkSize.SMALL -> stringResource(R.string.navmark_size_small)
                            app.pursi.ui.viewmodel.NavmarkSize.MEDIUM -> stringResource(R.string.navmark_size_medium)
                            app.pursi.ui.viewmodel.NavmarkSize.LARGE -> stringResource(R.string.navmark_size_large)
                        }
                        FilterChip(
                            selected = selected,
        ...
        item {
            Card(Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface)) {
                Column(Modifier.padding(16.dp)) {
                    ...
                    val continents = app.pursi.map.PmtilesDownloader.CONTINENTS
        ...
        item {
            Card(Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface)) {
                Column(Modifier.padding(16.dp)) {
                    ...
                    Text(
                        "Rakennettu ${BuildConfig.BUILD_TIMESTAMP}",
                        style = MaterialTheme.typography.bodySmall,
                        color = MaterialTheme.colorScheme.onSurface.copy(alpha = 0.5f)
                    )
                    Spacer(Modifier.height(4.dp))
